...
export function CourtsView({ courts, highlightCourtId, onHighlightClear, onAddCourt, onAddGroup, onDeleteCourt, onDeleteGroup, onEditGroup, onRateCourt, onAddReview }: CourtsViewProps) {
  ...
  return (
    <div className="max-w-screen-sm mx-auto px-4 pt-5 pb-10 sm:max-w-screen-2xl sm:px-10">
        ...
        <>
          ...
          <div className="sm:grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 sm:gap-3 sm:mb-6 flex gap-2 overflow-x-auto pb-2 mb-4 scrollbar-none">
            {filteredCourts.map(court => {
              ...
              const isPanelOpen = panelCourtId === court.id;
              return (
                <div key={court.id} ref={el => { courtRefs.current[court.id] = el; }} className="relative flex-shrink-0 w-48 sm:w-auto">
                  <button
                    onClick={() => { setSelectedCourtId(isSelected ? null : court.id); setPanelCourtId(court.id); }}
                    onMouseDown={e => e.preventDefault()}
                    className="relative text-left rounded-2xl px-4 py-3 overflow-hidden transition-all w-full"
                    style={{
                      background: 'linear-gradient(135deg, #0f172a, #1e3a5f)',
                      opacity: isSelected ? 1 : 0.65,
                      boxShadow: isSelected ? '0 8px 24px rgba(0,0,0,0.3)' : undefined,
                      outline: isSelected && !isPanelOpen ? '2px solid #84cc16' : 'none',
                    }}
          ...
          {/* Selected court groups — mobile only */}
          {selectedCourt && (
      ...
      {(() => {
        const panelCourt = courts.find(c => c.id === panelCourtId) ?? null;
        return (
          <CourtDetailPanel
            court={panelCourt}
            isOpen={!!panelCourtId}
            onClose={() => setPanelCourtId(null)}
            onRateCourt={() => panelCourt && onRateCourt(panelCourt.id)}
            onAddGroup={(defaultDay) => panelCourt && onAddGroup(panelCourt.id, defaultDay)}
            onDeleteGroup={(groupId) => panelCourt && onDeleteGroup(panelCourt.id, groupId)}
            onEditGroup={(groupId) => panelCourt && onEditGroup(panelCourt.id, groupId)}
            onAddReview={(groupId, notes) => panelCourt && onAddReview(panelCourt.id, groupId, notes)}
          />
        );
      })()}
