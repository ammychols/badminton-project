...
export function AddGroupModal({ courtName, defaultDay, initialValues, onClose, onSave }: AddGroupModalProps) {
  ...
  return (
    <BottomSheet title={initialValues ? 'แก้ไขก๊วน' : 'เพิ่มก๊วน'} onClose={onClose} footer={saveButton}>
      <p className="text-xs text-[var(--text-3)] -mt-3 mb-4">{courtName}</p>
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-[var(--text-2)] mb-2">รูปก๊วน (ไม่บังคับ)</label>
        {image ? (
          ...
          <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-[var(--dashed)] rounded-xl cursor-pointer hover:bg-[var(--hover-bg)] transition-colors">
            ...
            <span className="text-xs text-[var(--text-3)]">กดเพื่อเลือกรูป</span>
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-[var(--text-2)] mb-2">วันที่เปิด *</label>
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-[var(--text-2)] mb-2">มือ (ระดับ)</label>
        <div className="flex gap-2 flex-wrap">
          {ALL_LEVELS.map(lv => (
